...
- **Aspect ratio Instagram**: Todas as imagens (single e carrossel) passam por redimensionamento/recorte para o feed: aspect entre **4:5 e 1,91:1**, lado máximo 1080px (`toInstagramFeedImage` em `instagramImage.ts`). Evita erro de aspect ratio na publicação.
- **Texto no carrossel**: `POST /api/postador/carousel-adicionar-texto` (body: `image_urls`, `texts`) adiciona texto em cada imagem. Por padrão usa **Sharp** (overlay SVG). Se `POSTADOR_CAROUSEL_TEXTO_IA=gemini` e `GEMINI_API_KEY` estiverem definidos, tenta **Gemini/Imagen** (`editImage`, modelo configurável via `POSTADOR_CAROUSEL_TEXTO_IA_MODEL`, padrão `imagen-3.0-capability-001`). No painel, na revisão do carrossel, o usuário digita um texto por slide e clica em «Aplicar texto nas imagens».
- **Várias contas Instagram**: a config passa a ter `contas_instagram` (array de `{ id, nome, access_token, ig_user_id }`) e `instagram_default_id`. Em Administração o usuário adiciona/edita/remove contas e define a padrão. Ao publicar (ou publicar agendado), o painel envia `conta_id` opcional; o backend usa `getContaParaPublicar(config, conta_id)` para obter token e ig_user_id.
- **Imagen (Google)**: além de DALL·E, o endpoint `POST /api/postador/gerar-imagem` aceita `provider: "gemini"` para gerar imagem com **Imagen** (modelo `imagen-4.0-generate-001`). Requer `GEMINI_API_KEY`. No painel, ao marcar «Criar mídia com IA», o usuário pode escolher «Imagen (Google)» ou «DALL·E (OpenAI)». Vídeo com Veo (Gemini) é possível via API mas exige fluxo assíncrono (operação longa); fica como melhoria futura.
