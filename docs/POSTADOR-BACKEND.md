...
- **Texto no carrossel**: `POST /api/postador/carousel-adicionar-texto` (body: `image_urls`, `texts`) sobrepõe um texto em cada imagem (SVG + Sharp), faz upload no Cloudinary e retorna as novas URLs. No painel, na revisão do carrossel, o usuário pode digitar um texto por slide e clicar em «Aplicar texto nas imagens».
