...
A API **cria as tabelas na subida** (ao iniciar, se `DATABASE_URL` estiver definida). Se por algum motivo não tiverem sido criadas, rode manualmente no PostgreSQL:

```bash
psql -h HOST -U USER -d DBNAME -f api/migrations/001_postador_tables.sql
```

Ou copie o conteúdo de `api/migrations/001_postador_tables.sql` e execute no seu cliente SQL.
